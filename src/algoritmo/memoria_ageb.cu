...
void alojaMemAGEB(vector<Ageb> &vagb, PDEP &host_dep, PDEP &device_dep, PDRefP &host_drefp, PDRefP &device_drefp);
void liberaMemAGEB(PDEP &host_dep, PDEP &device_dep, PDRefP &host_drefp, PDRefP &device_drefp);
 ...
 * @param device_drefp
 */
void alojaMemAGEB(vector<Ageb> &vageb, PDEP &host_dep, PDEP &device_dep, PDRefP &host_drefp, PDRefP &device_drefp)
{

    size_t tam_pol = vageb.size();
    size_t tam_coord = 0;
    ...
    for (size_t i = 0; i < tam_pol; i++)
    {
        ...
        size_t tam_vp = vageb[i].vp.size();
        for (size_t j = 0; j < tam_vp; j++)
        {
            (host_dep + pos + j)->x = vageb[i].vp[j].x;
            (host_dep + pos + j)->y = vageb[i].vp[j].y;
        }
